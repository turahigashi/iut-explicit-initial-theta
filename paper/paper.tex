...
\date{Version \PaperVersion\ --- 12 September 2026\\[3pt]
...
\textbf{Supersession notice (version~0.1.7).}  Sections~7--9 below record the
source application proposed under the hypothesis \textup{(SA)}.  \textbf{That
application is withdrawn}: the companion note (``An explicit initial
$\Theta$-datum with mixed reduction at a split prime'', v0.1.7, \S7.1) shows that
version~0.1.6 did not establish an identification of the kind its
\textup{(F2)} describes, and withdraws it.  This is \emph{not} a proof that
\textup{(SA)} is false.  The exact rational calculations below stand; what does
not is the identification of which source quantity they are compared with.  Read
\S\S7--9 as the record of a reading that did not hold.
}}\end{center}
\medskip
...
sentences it rests on and exactly what would falsify it.  We draw no conclusion about the $abc$ inequality.  Nor do we claim
...
\paragraph{Relation to the first report.}
This paper is the \emph{expanded version} of \cite{First}, which states the datum and
its properties as its Theorem 1.1.  \emph{\cite{First} is the version of record}: it is
published with a DOI, whereas the present text is a draft circulated with it.  Where the
two differ, \cite{First} governs.  What is added here is the verification in full --- the
conditions of \cite[Definition 3.1]{IUT1} one by one, the image of the mod-$7$
representation, the $K$-core, the existence of the log-theta-lattice, and the arithmetic
existence family of Proposition~\ref{prop:exist} --- together with a comparison that is
\emph{conditional} on the hypothesis \SA{} of \S\ref{sec:sa}.  A reader who wants only
the unconditional content should read \S\S\ref{sec:datum}--\ref{sec:packet} and
Proposition~\ref{prop:exist}; \S\S\ref{sec:sa}--\ref{sec:averaged} assume \SA{}.
...
obtaining such consequences.  Dupuy and Hilado work out initial $\Theta$-data for the curve 11a1
...
The present paper contributes an explicit initial
...
\item any conclusion about the $abc$ inequality, or the falsity of any Diophantine consequence
      asserted in \cite{IUT4};
