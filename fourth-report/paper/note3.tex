...
\date{Version 1.0.4 --- 12 September 2026\\[3pt]
...
\textup{[P], added in version~1.0.4.}  \cite{First} has withdrawn, in its
version~0.1.7, the source application under which the values $8/7$ and $88/7$
were read as an incompatibility with \cite{IUT4}; see \cite[\S7]{First} for the
source locators and for a proposition limiting the scope of that withdrawal to
the mechanism used there.  \emph{No statement of the present report used that
application}: the passage computing those values states that whether the
expression of \cite[Prop.~1.4(iii)]{IUT4} governs the actual comparison region is
a separate question, not decided here, and that the computation does not refute
that proposition for the inputs covered by its hypotheses.  The numerical values
are unchanged, and nothing in this report is withdrawn.
