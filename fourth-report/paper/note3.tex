...
\date{Version 1.0.1 --- 8 September 2026\\[3pt]
\small ORCID \href{https://orcid.org/0009-0004-0460-6242}{\texttt{0009-0004-0460-6242}}\\[3pt]
\small DOI \href{https://doi.org/10.5281/zenodo.22648945}{10.5281/zenodo.22648945}}
...
that $\Ind2$ acts by a unit scalar and therefore leaves the generated hull
...
(xi-b)--(xi-f), and joining the two places was a reading, not a quotation (external
review, 2026-09-08).
...
compact, and the whole ambient object when it is not.  Then for every $g$ in the group
...
\emph{This is proved by exhibiting a common $B_t$-module, not inferred from equality
of the individual volumes.}  It concerns $\Ind2$ alone: it does not extend to
...
\textup{[S]} \emph{The same ratio carries the final inequality of} \cite{SS}.  There
the right-hand side of their (1.4) is averaged over the same range, giving the
coefficient $\frac{(\ell^{*}+1)(2\ell^{*}+1)}{12\ell}$ on $\deg(q_E)$ --- the form
\cite[Thm.~1.10, Step (v)]{IUT4} prints --- and solving for $\tfrac16\deg(q_E)$ yields
their (1.5),
\[
 \tfrac16\deg(q_E)\ \le\ \Bigl(1-\tfrac{12}{\ell(\ell+1)}\Bigr)^{-1}\cdot\mathfrak d(P).
\]
\textup{[P]} \emph{That factor is $\chi$}: the ratio of the stage-one coefficient
$1/(2\ell)$ to the averaged $(\ell+1)/24$ is $12/(\ell(\ell+1))$.  So Proposition
\ref{p:chi} is the identity on which \cite{SS}'s final inequality turns, and the
reading that the two sides use different normalizations is not available: the
coefficient is the one the source prints.

\textup{[S]} \emph{The source's own corresponding constant is a different one.}  In
assembling the local bounds of Steps (v)--(vii), \cite[Thm.~1.10]{IUT4} uses
...
$\frac{\ell+1}{2\ell^{2}}-\frac1{2\ell}=\frac1{2\ell^{2}}$, so the source \emph{bounds}
where \cite{SS} computes exactly.  \emph{What this records is that the averaging
...
match that drops mathematics and ignores punctuation, and reports eleven of the forty
